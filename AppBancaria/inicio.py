...
def abrir_pagina_cuenta(cuenta, cuentas_file, movimientos_file, parent=None):
    ...
    # saldo y movimientos
    saldo_var = tk.StringVar(value=f"${round(float(cuentas[idx]['saldo']), 2)}")
    ...
    def actualizar_saldo():
        ...
        idx_local = next((i for i, c in enumerate(cuentas_actualizadas) if c["usuario"] == usuario), None)
        if idx_local is not None:
            saldo_var.set(f"${round(float(cuentas_actualizadas[idx_local]['saldo']), 2)}")

    def actualizar_movimientos():
        lista_mov.delete(0, tk.END)
        movimientos_actuales = load_json(movimientos_file, {})
        for mov in movimientos_actuales.get(usuario, []):
            lista_mov.insert(tk.END, f"{mov['fecha']} - {mov['tipo']} ${mov['monto']} - {mov.get('nota','')}")
    ...
    def abrir_pago_servicios():
        pago_servicios(
            ...
            on_update=lambda: (actualizar_movimientos(), actualizar_saldo())
        )
    ...
    tk.Button(frame_btn, text="Deposito/Retiro", font=("Arial", 14, "bold"),
              command=lambda: abrir_deposito_retiro(
                  parent=cuenta_win,
                  cuenta=cuenta,
                  cuentas_file=cuentas_file,
                  movimientos_file=movimientos_file,
    ...
    tk.Button(frame_btn, text="Pago de servicios", font=("Arial", 14, "bold"),
              command=abrir_pago_servicios, bd=7, width=18, bg="red").grid(row=0, column=1, padx=10)

    tk.Button(frame_btn, text="Compra de divisas", font=("Arial", 14, "bold"),
              command=lambda: programa_intercambio(
                  parent=cuenta_win,
                  usuario=usuario,
                  on_update=lambda: (actualizar_movimientos(), actualizar_saldo())
              ),
              bd=7, width=18, bg="red").grid(row=0, column=2, padx=10)
