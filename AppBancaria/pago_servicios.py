...
def pago_servicios(parent, cuenta, cuentas, idx, cuentas_file, movimientos, movimientos_file, on_update=None):
    """
    parent: ventana padre
    cuenta: dict del usuario actual
    cuentas: lista cargada (puede estar desactualizada; funciones vuelven a leer archivos cuando guardan)
    idx: índice de la cuenta en la lista cuentas pasada (se revalida antes de guardar)
    cuentas_file: ruta absoluta al JSON de cuentas
    movimientos: dict de movimientos (puede estar desactualizado)
    movimientos_file: ruta absoluta al JSON de movimientos
    on_update: callback a ejecutar cuando se hayan realizado cambios (ej. actualizar lista y saldo)
    """
    ...
    # servicios provistos por default (si existe servicios.json lo usamos)
    servicios_file = os.path.join(BASE_DIR, "servicios.json")
    if os.path.exists(servicios_file):
        try:
            servicios_data = load_json(servicios_file, [])
            servicios = [[s["codigo"], s["nombre"], s["monto"]] for s in servicios_data]
        except Exception:
            servicios = [
                ["100", "AYSA", 500],
                ["200", "Metrogas", 750],
    ...
    def verificar_saldo(monto):
        # recargar cuentas actuales antes de verificar
        cuentas_local = load_json(cuentas_file, [])
        idx_local = next((i for i, c in enumerate(cuentas_local) if c["usuario"] == cuenta["usuario"]), None)
        if idx_local is None:
            return False
        return cuentas_local[idx_local]["saldo"] >= monto

    def descontar_saldo(monto, concepto):
        cuentas_local = load_json(cuentas_file, [])
        movimientos_local = load_json(movimientos_file, {})
        idx_local = next((i for i, c in enumerate(cuentas_local) if c["usuario"] == cuenta["usuario"]), None)
        if idx_local is None:
            messagebox.showerror("Error", "Cuenta no encontrada.")
            return False

        if cuentas_local[idx_local]["saldo"] < monto:
            messagebox.showerror("Error", "Saldo insuficiente")
            return False

        cuentas_local[idx_local]["saldo"] = round(float(cuentas_local[idx_local]["saldo"]) - monto, 2)
        fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        mov = {"tipo": "pago", "monto": monto, "fecha": fecha, "nota": concepto}
        movimientos_local.setdefault(cuenta["usuario"], []).append(mov)

        save_json_atomic(cuentas_file, cuentas_local)
        save_json_atomic(movimientos_file, movimientos_local)

        # ejecutar callback para que la ventana principal refresque
        if on_update:
            try:
                on_update()
            except Exception:
    ...
    def continuar():
        ...
        if servicio_actual:
            ...
            cuentas_local = load_json(cuentas_file, [])
            idx_local = next((i for i, c in enumerate(cuentas_local) if c["usuario"] == cuenta["usuario"]), None)
            saldo_actual = cuentas_local[idx_local]["saldo"] if idx_local is not None else 0
            tk.Label(root, text=f"Saldo actual: ${round(float(saldo_actual),2)}", font=("Arial", 12)).pack(pady=5)
            ...
            tk.Button(root, text="Volver", command=pagar_servicios_menu).pack(pady=10)
        else:
    ...
    def pagar(metodo):
        ...
        confirmar = messagebox.askyesno(
            "Confirmar",
            f"¿Pagar ${monto} con {metodo}?\n\nSaldo actual: ${round(float(saldo_actual),2)}\nSaldo después: ${round(float(saldo_actual) - monto,2)}"
        )
        if confirmar:
            if descontar_saldo(monto, f"Pago {nombre} via {metodo}"):
    ...
    def pedir_monto_celular():
        ...
        numero_celular = recarga_numero.get().strip()
    ...
    def confirmar_recarga(monto):
        ...
        confirmar = messagebox.askyesno(
            "Confirmar",
            f"¿Recargar ${monto} a {empresa_seleccionada} - {numero_celular}?"
        )
        if confirmar:
    ...
    def mostrar_montos_sube():
        ...
        numero_sube = sube_numero.get().strip()
    ...
    def confirmar_sube(monto):
        ...
        confirmar = messagebox.askyesno(
            "Confirmar",
            f"¿Cargar SUBE {numero_sube} con ${monto}?"
        )
        if confirmar:
    ...
    def mostrar_menu():
        ...
        idx_local = next((i for i, c in enumerate(cuentas_local) if c["usuario"] == cuenta["usuario"]), None)
        saldo_actual = cuentas_local[idx_local]["saldo"] if idx_local is not None else 0
        tk.Label(root, text=f"Saldo disponible: ${round(float(saldo_actual),2)}", font=("Arial", 12, "bold")).pack(pady=5)
        tk.Button(root, text="Pagar Servicios", width=25, command=pagar_servicios_menu).pack(pady=10)
