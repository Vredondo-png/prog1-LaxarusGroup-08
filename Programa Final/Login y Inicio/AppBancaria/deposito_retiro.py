...
def obtener_codigo_ubicacion(codigo_banco, nombre_ubicacion, ubicaciones_data):
...
# === FUNCIÓN PRINCIPAL PARA ABRIR DESDE INICIO.PY ===
def abrir_ventana_deposito_retiro(parent, cuenta, cuentas_file, movimientos_file, callback_actualizar=None):
    """
    Abre la ventana de depósito/retiro integrada con el sistema de cuentas
    
    parent: ventana padre
    cuenta: dict con datos de la cuenta actual
    cuentas_file: ruta al archivo cuentas.json
    movimientos_file: ruta al archivo movimientos.json
    callback_actualizar: función para actualizar la ventana padre después de operaciones
    """
...
# Mantener la función original para ejecutar standalone
def ejecutar_aplicacion():
    root = tk.Tk()
    root.withdraw()
    messagebox.showinfo("Información", "Este módulo debe ser ejecutado desde el sistema principal de login")
    root.destroy()
